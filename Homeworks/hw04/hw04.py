...
def make_withdraw(balance, password):
    ...
    """
    incorrect_lists = []
    def withdraw(account,inputpass):
        nonlocal incorrect_lists,balance,password
        if len(incorrect_lists) == 3:
            return "Your account is locked. Attempts: ['{}', '{}', '{}']".format(incorrect_lists[0],incorrect_lists[1],incorrect_lists[2])
        if inputpass != password:
            if inputpass not in incorrect_lists:
                incorrect_lists +=[inputpass]
...
class Mint:
    ...
    def create(self, kind):
        new_coin = kind(self.current_year)
        return new_coin
...
class Coin:
    ...
    def worth(self):
        worthy = cents + (Mint.current_year-self.year) - 50
        return worthy
