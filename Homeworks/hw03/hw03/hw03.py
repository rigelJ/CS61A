...
def balanced(m):
    ...
    s1 = left(m)
    s2 = right(m)
    if total_weight(end(s1))*length(s1) == total_weight(end(s2))*length(s2):
        if is_weight(end(s1)):
            a = True
        else:
            a= balanced(end(s1))
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        lsp = []
        lsp += [totals_tree(end(left(m)))]
        lsp += [totals_tree(end(right(m)))]
        return tree(total_weight(m),lsp)
...
def one(f):
    return lambda x:f(x)  # return lambda x:f(zero(f)(x)) 
    

def two(f):
    return lambda x:f(f(x)) #return lambda x:f(one(f)(x))

#def n(f):
    #return lambda x:f(f(f..(x))) return lambda x:f(n-1(f)(x))
    #N = n(f)(x)
...
def add_church(m, n):
    ...
    #m(f) return lambda x : f(f(x)) n(f) lambda x:f(f(f(x)))
    return lambda f: lambda x: n(f)(x) +m(f)(x)

    #这里告诉我们，如果我们需要将参数带入两个不同式子，那就构造一个函数，将参数同时带入两个式子
