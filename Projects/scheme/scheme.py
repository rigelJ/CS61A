...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    formals_list = expressions.first
    body_list = expressions.second
    return LambdaProcedure(formals_list,body_list,env)
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_formals(formals)
        values = Pair(scheme_eval(expr.second.first,env),values)
...
def do_mu_form(expressions, env):
    ...
    formals_list = expressions.first
    body_list = expressions.second
    return MuProcedure(formals_list,body_list)
    # END PROBLEM 16
